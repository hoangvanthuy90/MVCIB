...
from util import getM_logM, load_dgl_fromPyG
...
def run(i, dataset_full, num_features, num_classes):
    ...
    print("\n After Training Graphs: ", len(trainset))
    print("Validation Graphs: ", len(valset))
    print("Test Graphs: ", len(testset))
...
from train_moleculenetBACE import train_epoch, evaluate_network, train_epoch_graph_classification, \
...
def generate_graphs_new(dataset):
    if args.dataset in ["BBBP"]:
        file_name = "original_datasets/BBBP/bbbp/raw/BBBP.csv"
        data = pd.read_csv(file_name)
        d_smile = data['smiles']
        d_class = data['p_np']
    else:
        file_name = "original_datasets/BACE/bace/raw/BACE.csv"
        data = pd.read_csv(file_name)
        d_smile = data['mol']
        d_class = data['Class']
    ...
    for i, row in data.iterrows():
        ...
        smile = d_smile[i]
        y = d_class[i]
        # print(f"smile: {smile}") ; print(f"y: {y}") ; raise SystemExit()
        # checking
        #data_check = dataset[i]
        # if y != data_check.y:
        #     print(f" error data_check {y} {data_check.y}")
            #raise SystemExit()
 
        y_tensor = torch.tensor(y).view(1, -1)
        # print(f"data_check.y: {data_check.y.type} | {data_check.y} | {y} | {y_tensor}")
        # raise SystemExit()
        ...
        try:
            ...
            # print(f"coords_tensor {coords_tensor.shape} | {coords_tensor[0]}")
            # raise SystemExit()
...
if __name__ == '__main__':
    ...
    # BACE		BBBP
    # python exp_moleculenetBACE_BBBP.py --dataset BBBP  --device cuda:0
