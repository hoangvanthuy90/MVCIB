...
from train_molqm9 import train_epoch, evaluate_network, train_epoch_graph_classification, train_epoch_domainadaptation
...
def main():
    ...
    args.num_classes = 12  # dataset.num_classes #dataset.num_tasks
...
def generate_graphs_new():
    if args.dataset in ["QM9"]:  # FreeSolv  ESOL
        file_name = "original_datasets/QM9/raw/gdb9.sdf.csv"
        data = pd.read_csv(file_name)
        d_smile = data['smiles']
        d_class = data.iloc[:, 4: 16]
    else:
...
def load_graphdataset(dataset_name):
    ...
    for i in range(len(graph_lists)):
        ...
        current_label = current_label[args.target_index]
