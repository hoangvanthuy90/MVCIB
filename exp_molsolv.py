...
def generate_graphs_new(dataset):
    if args.dataset in ["FreeSolv"]:  # FreeSolv  ESOL
        file_name = "original_datasets/FreeSolv/freesolv/raw/SAMPL.csv"
        data = pd.read_csv(file_name)
        d_smile = data['smiles']
        d_class = data.iloc[:, 2]
    elif args.dataset in ["ESOL"]:
        file_name = "original_datasets/ESOL/esol/raw/delaney-processed.csv"
        data = pd.read_csv(file_name)
        d_smile = data['smiles']
        d_class = data.iloc[:, 8]
    elif args.dataset in ["Lipo"]:
        file_name = "original_datasets/Lipo/lipo/raw/Lipophilicity.csv"
        data = pd.read_csv(file_name)
        d_smile = data['smiles']
        d_class = data.iloc[:, 1]
    ...
    for i, row in data.iterrows():
        ...
        try:
            ...
            graph_labels.append(y_tensor.float())
            data = dataset[i];
            y_py = data.y
...
if __name__ == '__main__':
    ...
    parser.add_argument("--pt_epoches", type=int, default=450)
    parser.add_argument("--ft_epoches", type=int, default=450)
